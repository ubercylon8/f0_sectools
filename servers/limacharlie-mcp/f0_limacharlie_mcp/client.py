...
class LimaCharlieClient:
    ...
    def org_stats(self) -> dict:
        return self._org.get_stats()

    def list_sensors(self, online_only: bool = False, limit: int = 50) -> list[dict]:
        return list(self._org.list_sensors(is_online_only=online_only, limit=limit))

    def find_sensor(self, hostname: str) -> dict:
        return self._org.find_sensors_by_hostname(hostname)

    def list_dr_rules(self, namespace: str = "general") -> dict:
    ...
    def query(self, lcql: str, start: int, end: int, limit: int = 50) -> list[dict]:
