...
| `distribution/config.yaml` | The nine f0_sectools MCP servers (defender, entra, limacharlie, projectachilles, pa-actions, intune, tenable, purview, sentinel) under `mcp_servers`, plus personas and skills — auto-installed to `~/.hermes/profiles/f0sectools/config.yaml`. Hermes reads MCP servers from `config.yaml`, not a separate `mcp.json`. |
