...
Current skills: `defender/{triage-incident,posture-summary,threat-hunt}`, `entra/{identity-risk-review,conditional-access-audit,privileged-access-review}`, `limacharlie/{detection-coverage-review,endpoint-investigation,threat-hunt}`. A test (`skills/test_skills_valid.py`) enforces valid frontmatter and the ≤60-char description limit on every `SKILL.md`.
